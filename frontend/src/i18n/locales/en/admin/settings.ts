export default {
    settings: {
      ...
      scheduling: {
        ...
        stickyOverflowSlotsTitle: 'Sticky session concurrency overflow slots',
        stickyOverflowSlotsDescription: 'When a sticky session hits an account whose slots are full, retry the acquire under a relaxed limit of "account concurrency + N" and forward immediately if it succeeds, skipping the sticky wait queue. Only if that also fails does it fall back to the existing queue/failover path. 0 = disabled, max 5. Applies only to sticky hits; regular selection still respects the account concurrency limit.',
        stickyOverflowSlotsWarning: 'Concurrency limits exist to protect upstream. Relaxing them raises the chance of upstream 429s and rate limiting — start at 1 and observe.',
